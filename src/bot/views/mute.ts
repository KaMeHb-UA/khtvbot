...
export default async ({ groupId }: ViewArgs): Promise<View> => {
	...
	lastUpdates.filter((update) => !admins.includes(update.uid!)).slice(0, lastUsersLimit).forEach((update, i) => {
		...
		lastRow.push(new TGInlineButton(btnText, `${OPCODE.MUTE_CONFIRM}${OPCODE.B64_TO_INT}:${intToBase64(updateFrom.id)}`, false));
	});
	return {
		...
		buttons: [
			...dynamicRows,
			[
				new TGInlineButton('admin_search_cmd', `${OPCODE.MUTE_CONFIRM}${OPCODE.SEARCH_USER}`),
			],
			[
